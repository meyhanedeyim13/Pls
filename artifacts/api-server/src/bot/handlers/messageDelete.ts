...
export function registerMessageDelete(client: Client): void {
  client.on(Events.MessageCreate, (message) => {
    if (!(CONFIG.PROTECTED_CHANNEL_IDS as readonly string[]).includes(message.channelId)) return;
      ...
      if (cached) {
        ...
        if (restoreEmbeds.length > 0 || cached.attachmentUrls.length > 0) {
          ...
          if (cached.components.length > 0) {
            sendOptions.components = cached.components
              .filter((row) => row?.type === 1)
              .map((row) => ActionRowBuilder.from<ButtonBuilder>(row));
          }
